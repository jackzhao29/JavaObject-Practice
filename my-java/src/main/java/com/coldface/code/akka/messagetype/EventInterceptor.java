...
/**
 * 类EventInterceptor.java的实现描述：TODO 类实现描述
 * 
 * @author coldface
 * @date 2016年7月17日上午10:34:51
 */
public class EventInterceptor extends ClusterRoleWorker {
  ...
  private List<String> blackIpList =
      Arrays.asList("5.9.116.101", "103.42.176.138", "123.182.148.65", "5.45.64.205",
          "27.159.226.192", "76.164.228.218", "77.79.178.186", "104.200.31.117", "104.200.31.32",
          "104.200.31.238", "123.182.129.108", "220.161.98.39", "59.58.152.90", "117.26.221.236",
          "59.58.150.110", "123.180.229.156", "59.60.123.239", "117.26.222.6", "117.26.220.88",
          "59.60.124.227", "142.54.161.50", "59.58.148.52", "59.58.150.85", "202.105.90.142");
    ...
    if (message instanceof ClusterEvent.MemberUp) {
      ...
      for (Member member : members) {
        if (member.status() == MemberStatus.up()) {
          register(member, getCollectorPath(member));
        }
      }
      ...
      if (!checkRecord.isIpInBlackList) {
        ...
        if (workers.size() > 0) {
          ...
          workers.get(processorIndex)
              .tell(
                  new EventMessages.FilteredRecord(nginxRecord.getSourceHost(),
                      nginxRecord.getLine(), nginxRecord.getEventCode(),
                      checkRecord.data.get("eventdate"), checkRecord.data.get("realip")),
                  getSelf());
